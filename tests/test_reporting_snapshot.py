...
class TestReportingSnapshot(unittest.TestCase):
    ...
    def test_capture_snapshot_accepts_newer_live_surface_when_health_matches(self):
        with tempfile.TemporaryDirectory() as tmp:
            base_dir = Path(tmp)
            workspace_dir = base_dir / "workspace"
            workspace_dir.mkdir()
            ...
            snapshot = capture_report_snapshot(
                state={"timestamp": 120.0, "fill_ratio": 0.61, "eig1": 24.0},
                session_id=12,
                base_dir=base_dir,
                workspace_dir=workspace_dir,
            )

            self.assertTrue(snapshot.health.valid_for_state)
            self.assertTrue(snapshot.spectral.valid_for_state)
            self.assertEqual(snapshot.state["fill_pct"], 84.1)
            summary = format_snapshot_summary(snapshot)
            provenance = format_snapshot_provenance(snapshot)
            self.assertIn("DB state refreshed from live surface", summary)
            self.assertIn("DB state refreshed from live surface", provenance)
