...
class TestAutonomousAgentActionContinuity(unittest.TestCase):
    ...
    def test_direct_pressure_uses_private_canvas_and_rest_keeps_journal_contract(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("Optional own-journal context", pressure_prompt)
            self.assertNotIn("Continuity posture:", pressure_prompt)
            self.assertEqual(pressure_prompt.count("State anchor:"), 1)
            self.assertNotIn("sand/silt/grit", pressure_prompt)
            self.assertNotIn("Lead with felt texture", pressure_prompt)
    ...
    def test_moment_capture_uses_qualia_lane_and_delimited_body(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("Measured state at capture:", captured["prompt"])
            self.assertNotIn("afterimage", captured["prompt"])
            self.assertNotIn("Fresh telemetry anchors", captured["prompt"])
    ...
    def test_moment_capture_labels_marker_age_and_current_cov_lambda(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(handled)
            self.assertIn("record_age=82s ago (1m 22s; at prompt capture)", captured["prompt"])
            self.assertIn("lambda1_esn=14.931", captured["prompt"])
            self.assertIn("lambda1_cov=7.980", captured["prompt"])
            self.assertIn("current_fill_frame=upper boundary / elevated edge", captured["prompt"])
            self.assertIn("present effects unknown", captured["prompt"])
            self.assertNotIn("now plus echo", captured["prompt"])
